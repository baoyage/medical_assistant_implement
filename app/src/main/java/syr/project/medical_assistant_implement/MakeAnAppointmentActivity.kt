...
DoctorsRecommendedFragment.OnRecyclerInteractionListener, Communicator{
    ...
    override fun onSpecialtyClicked(){
        val appBar = supportActionBar
        supportFragmentManager.beginTransaction().replace(R.id.makeAnAppointmentContainer, DoctorsRecommendedFragment.newInstance()
        ).addToBackStack(null).commit()
    }
    override fun onDoctorClicked(){
        val appBar = supportActionBar
        supportFragmentManager.beginTransaction().replace(R.id.makeAnAppointmentContainer, DoctorSelectedFragment.newInstance()
        ).addToBackStack(null).commit()
    }
